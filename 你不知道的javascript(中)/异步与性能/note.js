...
// 任务分为同步和异步，异步里面区分宏观任务，微观任务,
// javascript执行栈没有要执行的代码的时候，会先去微任务队列里面找，如果微任务队列执行完了。会去找异步任务里的宏观任务队列，宏观任务执行完之后，会开始执行另外的宏观任务，然后再执行这个宏观任务里微任务，执行完之后，再看微任务队列有没有任务，没有的话，再去执行一个新的宏观任务。
// 总之，在执行栈中的任务清空了之后，总是先找微任务队列。微任务队列为空的时候，才回去执行宏任务队列里的任务。
